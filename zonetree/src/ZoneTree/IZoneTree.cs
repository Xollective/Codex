...
public interface IZoneTree<TKey, TValue> : IDisposable
{
    ...
    bool TryAtomicAddOrUpdate<TArg>(in TKey key, in TArg arg, ValueUpdaterDelegate<TValue, TArg> valueUpdater);
...
/// <param name="value">The value as a reference to be updated.</param>
/// <returns>true if the value is updated, false otherwise.</returns>
public delegate bool ValueUpdaterDelegate<TValue, TArg>(ref TValue value, in TArg arg);

public ref struct RefVar<T>
{
    public ref T Value;
}
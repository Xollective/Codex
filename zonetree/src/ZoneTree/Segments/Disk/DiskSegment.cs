...
public sealed class DiskSegment<TKey, TValue> : IDiskSegment<TKey, TValue>
{
        ...
        ZoneTreeOptions<TKey, TValue> options)
    {
        ...
        HasFixedSizeKey = KeySerializer.IsFixedSize;
        HasFixedSizeValue = ValueSerializer.IsFixedSize;
        ...
        KeySize = KeySerializer.FixedSize;
        ValueSize = ValueSerializer.FixedSize;
        if (HasFixedSizeKeyAndValue)
        {
    ...
    unsafe TKey ReadKey(long index)
    {
        try
        {
            ...
            if (HasFixedSizeKeyAndValue)
            {
                ...
                return KeySerializer.SpanDeserialize(keyBytes);
            }
            else if (HasFixedSizeKey)
            {
    ...
    unsafe TValue ReadValue(long index)
    {
        try
        {
            ...
            if (HasFixedSizeKeyAndValue)
            {
                ...
                return ValueSerializer.SpanDeserialize(valueBytes);
            }
            else if (HasFixedSizeKey)
            {
                var headSize = sizeof(ValueHead) + KeySize;

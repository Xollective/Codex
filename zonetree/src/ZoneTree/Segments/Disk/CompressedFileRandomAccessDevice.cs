...
public sealed class CompressedFileRandomAccessDevice : IRandomAccessDevice
{
    ...
    public long AppendBytesReturnPosition(ReadOnlySpan<byte> bytes)
    {
        var pos = GetLength();
        ...
        {
            copiedBytes += AppendBytesInternal(bytes.Slice(copiedBytes));
        }
        return pos;
    }
    ...
    public ReadOnlyMemory<byte> GetBytes(long offset, int length)
    {
        ...
        if (offsetInBlock + length > BlockSize)
        {
            ...
            chunk1.CopyTo(bytes.AsMemory(0, chunk1.Length));
            chunk2.CopyTo(bytes.AsMemory(chunk1.Length));
            return bytes;
        }
        var block = NextBlock;
    ...
    ReadOnlyMemory<byte> ReadBlockAndAddToCache(
        int blockIndex,
        int offsetInBlock,
        int length)
    {
    ...
    public int GetBytes(long offset, Span<byte> buffer)
    {
        var bytes = GetBytes(offset, buffer.Length);
        bytes.Span.CopyTo(buffer.Slice(0, bytes.Length));
        return bytes.Length;
    }

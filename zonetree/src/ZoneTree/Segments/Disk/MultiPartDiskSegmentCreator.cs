...
public sealed class MultiPartDiskSegmentCreator<TKey, TValue> : IDiskSegmentCreator<TKey, TValue>
{
    ...
    void WriteKeys(BinaryWriter bw)
    {
        ...
        for (var i = 0; i < len; ++i)
        {
            ...
            var b = i;
            var k1 = PartKeys[a];
            var bytes = KeySerializer.SpanSerialize(k1);
            bw.Write(bytes.Length);
            bw.Write(bytes);

            var k2 = PartKeys[b];
            bytes = KeySerializer.SpanSerialize(k2);
            bw.Write(bytes.Length);
            bw.Write(bytes);
        }
    }
    ...
    void WriteValues(BinaryWriter bw)
    {
        ...
        for (var i = 0; i < len; ++i)
        {
            ...
            var b = i;
            var v1 = PartValues[a];
            var bytes = ValueSerializer.SpanSerialize(v1);
            bw.Write(bytes.Length);
            bw.Write(bytes);

            var v2 = PartValues[b];
            bytes = ValueSerializer.SpanSerialize(v2);
            bw.Write(bytes.Length);
            bw.Write(bytes);
        }
    }

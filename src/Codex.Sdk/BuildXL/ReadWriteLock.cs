...
namespace BuildXL.Utilities.Threading
{
    ...
    public readonly struct ReadWriteLock
    {
        ...
        public WriteLock AcquireWriteLock(bool allowReads = false, ReadLock readLock = default)
        {
            bool suspendReadLock = readLock.IsValid;
            if (suspendReadLock)
            {
                ExitReadLock();
            }
            ...
            return new WriteLock(this, allowReads, suspendedReadLock: suspendReadLock);
        }
    ...
    public readonly struct WriteLock : IDisposable
    {
        ...
        /// <summary>
        /// Indicates whether caller which acquired write lock specified
        /// that it held a read lock which was suspended for the scope of
        /// write lock
        /// </summary>
        private readonly bool m_suspendedReadLock;
        ...
        /// Constructor
        /// </summary>
        public WriteLock(ReadWriteLock rwLock, bool allowReads, bool suspendedReadLock = false)
        {
            m_lock = rwLock;
            m_allowReads = allowReads;
            m_suspendedReadLock = suspendedReadLock;

            Contract.Assert(!suspendedReadLock || IsValid, "Lock must be valid in order to suspend read lock");
        }
        ...
        public void Dispose()
        {
            if (IsValid)
            {
                ...
                // Resume the read lock if it was suspended.
                if (m_suspendedReadLock)
                {

...
namespace BuildXL.Utilities.Collections
{
    ...
    public sealed partial class ConcurrentBigSet<TItem> : ConcurrentBigSet
    {
        ...
        private IEqualityComparer<TItem> m_comparer;
        ...
        public ConcurrentBigSet(
            ...
            : this(
                  ...
                  capacity: capacity,
                  ratio: ratio,
                  itemsPerEntryBufferBitWidth: itemsPerEntryBufferBitWidth,
                  comparer: comparer)
        {
            ...
            IEqualityComparer<TItem> comparer = null)
        {
            ...
            m_comparer = comparer ?? EqualityComparer<TItem>.Default;
        ...
        public bool TryGetIndex(TItem item, out int index, IEqualityComparer<TItem> comparer = null)
        {
            var result = GetOrAdd(item, comparer: comparer, allowAdd: false);
            index = result.Index;
            return result.IsFound;
        }
        ...
        {
            comparer = comparer ?? m_comparer;
            return AddItem(new PendingSetItem<TItem, TItem>(item, comparer));
        }
        ...
        {
            comparer = comparer ?? m_comparer;
            return GetOrAddItem(new PendingSetItem<TItem, TItem>(item, comparer), allowAdd);
        }
        ...
        {
            comparer = comparer ?? m_comparer;
            return UpdateItem(new PendingSetItem<TItem, TItem>(item, comparer), allowAdd);
        }
        ...
        {
            comparer = comparer ?? m_comparer;
            return GetAddOrUpdateItem(new PendingSetItem<TItem, TItem>(item, comparer, remove: true), allowAdd: false, update: true);
        }
        ...
        private GetAddOrUpdateResult GetAddOrUpdateItem<TPendingItem>(in TPendingItem pendingItem, bool allowAdd = true, bool update = false)
            where TPendingItem : IPendingSetItem<TItem>
        {
            int bucketHashCode = GetBucketHashCode(pendingItem.HashCode);
        ...
        /// </summary>
        private GetAddOrUpdateResult FindItem<TPendingItem>(in TPendingItem pendingItem, int bucketHashCode, out int bucketNo, out int headNodeIndex, ref Accessors accessors, out int findCount, out int priorNodeIndex)
            where TPendingItem : IPendingSetItem<TItem>
        {
            findCount = 0;
...
#if DEBUG_BIG_SET
            SpanBuilder<TItem> items = StructArray.GetItemSpan<T64<TItem>, TItem>();
            SpanBuilder<Node> nodes = stackalloc Node[items.Capacity];
#endif
...
#if DEBUG_BIG_SET
                    items.TryAdd(retrievedItem);
#endif
#if DEBUG_LOCAL
                    Contract.Assert(findCount < 50);
#endif
        ...
        public readonly struct GetAddOrUpdateResult
        {
            ...
            public bool ItemWasAdded => !IsFound;
            ...
            /// </summary>
            public static GetAddOrUpdateResult NotFound => default(GetAddOrUpdateResult);
        ...
        /// the given comparer.
        /// </summary>
        public readonly struct PendingSetItem<TOther, TItem> : IPendingSetItem<TItem>
            where TItem : TOther
        {
            private readonly int m_hashCode;
            private readonly TOther m_item;
            private readonly IEqualityComparer<TOther> m_comparer;
            private readonly bool m_remove;

            public PendingSetItem(TOther item, IEqualityComparer<TOther> comparer, bool remove = false)
            {
                m_item = item;
            ...
            public PendingSetItem(TItem item, int hashCode, IEqualityComparer<TOther> comparer, bool remove = false)
            {
                m_item = item;
                m_hashCode = hashCode;
                m_comparer = comparer;

...
namespace Codex.Utilities.Tasks
{
    ...
    public readonly struct TaskSourceSlim<TResult>
    {
        ...
        public void LinkToTask<T, TData>(Task<T> task, TData data, Func<T, TData, TResult> getResult)
        {
            ...
            task.ContinueWith((Task continuation) => LinkToContinuation(task, (data, getResult), static (task, t) => t.getResult(task.Result, t.data), continuation, @this));
        }
        ...
        {
            LinkToContinuation(task, getResult, static (result, getResult) => getResult(result.Result), task, this);
        }

        /// <summary>
        /// Sets the task source to the result of the task
        /// </summary>
        public void TrySetFromTask(Task task, Func<TResult> getResult)
        {
            LinkToContinuation(task, getResult, static (result, getResult) => getResult(), task, this);
        }

        private static void LinkToContinuation(Task<TResult> task, Task continuation, TaskSourceSlim<TResult> @this)
        {
            LinkToContinuation(task, true, static (result, _) => result.Result, continuation, @this);
        }

        private static void LinkToContinuation<TTask, TData>(TTask task, TData data, Func<TTask, TData, TResult> getResult, Task continuation, TaskSourceSlim<TResult> @this)
            where TTask : Task
        {
            ...
            {
                @this.TrySetResult(getResult(task, data));
            }
        }

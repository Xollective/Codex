...
namespace Codex.Utilities
{
    public class SourceTextReader
    {
        ...
        private SubSourceText GetSubSourceText(int start, int length)
        {
            return new SubSourceText(_s, new TextSpan(start, length));
        }
        ...
        public SubSourceText ReadLine()
        {
            ...
            while (i < _length)
            {
                ...
                if (ch == '\r' || ch == '\n')
                {
                    var result = GetSubSourceText(_pos, i - _pos);
                    _pos = i + 1;

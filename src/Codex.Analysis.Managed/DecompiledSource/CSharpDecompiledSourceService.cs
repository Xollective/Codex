...
namespace Microsoft.CodeAnalysis.Editor.CSharp.DecompiledSource
{
    ...
    internal class CSharpDecompiledSourceService
    {
        ...
        public async Task<Document?> AddSourceToAsync(Document document, Compilation symbolCompilation, ISymbol symbol, MetadataReference? metadataReference, PEFile? file, SyntaxFormattingOptions formattingOptions, CancellationToken cancellationToken)
        {
            ...
            // Decompile
            var decompiledDocument = PerformDecompilation(document, fullName, symbolCompilation, metadataReference, file);
        ...
        public static async Task<Document> FormatDocumentAsync(Document document, SyntaxFormattingOptions options, CancellationToken cancellationToken)
        {
            ...
            var formattedDoc = await Formatter.FormatAsync(
                 ...
                 new[] { node.FullSpan },
                 options,
                 CSharpDecompiledSourceFormattingRule.Instance.Concat(Formatter.GetDefaultFormattingRules(document)).ToImmutableArray(),
                 cancellationToken).ConfigureAwait(false);

            return formattedDoc;
        }

        private static Document? PerformDecompilation(Document document, string fullName, Compilation compilation, 
            ...
            PEFile? file)
        {
            ...
            // Load the assembly.
            if (file is null && metadataReference is not null)
            ...
            var newRoot = oldRoot.WithLeadingTrivia(new[]
                {
                    ...
                    SyntaxFactory.CarriageReturnLineFeed,
                    SyntaxFactory.Comment($"// Decompiled with ICSharpCode.Decompiler"),
                    SyntaxFactory.CarriageReturnLineFeed,

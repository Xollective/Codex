...
namespace Codex.Lucene.Framework
{
    ...
    /// @lucene.internal
    /// </summary>
    public class FixedBitSet<TBits> : DocIdSet, IFixedBitSet
        where TBits : struct, IMemory<long>
    {
        public int? Count { get; set; }

        DocIdSet IFixedBitSet.DocIdSet => this;
        ...
        public sealed class FixedBitSetIterator : DocIdSetIterator
        {
            ...
            internal readonly TBits _bits;
            internal Span<long> bits => _bits.Span;
            internal int doc = -1;

            /// <summary>
            /// Creates an iterator over the given <see cref="FixedBitSet"/>. </summary>
            public FixedBitSetIterator(FixedBitSet<TBits> bits)
                : this(bits._bits, bits.numBits, bits.numWords)
            {
            }

            /// <summary>
            /// Creates an iterator over the given array of bits. </summary>
            public FixedBitSetIterator(TBits bits, int numBits, int wordLength)
            {
                this._bits = bits;
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static long IntersectionCount(FixedBitSet<TBits> a, FixedBitSet<TBits> b)
        {
        ...
        /// set is modified.
        /// </summary>
        public static long UnionCount(FixedBitSet<TBits> a, FixedBitSet<TBits> b)
        {
        ...
        /// </summary>
        public static long AndNotCount(FixedBitSet<TBits> a, FixedBitSet<TBits> b)
        {
        ...
        public FixedBitSet(TBits storedBits, int numBits)
        {
            ...
            this.numBits = numBits;
            this._bits = storedBits;
        }

        public override DocIdSetIterator GetIterator()
        {
            return new FixedBitSetIterator(_bits, numBits, numWords);
        }
        ...
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => Count ?? (int)BitUtil.Pop_Array(bits, 0, bits.Length);
        }

        public bool Get(int index)
        {
            if (Debugging.AssertsEnabled) Contract.Check(unchecked((uint)index < (uint)numBits))?.Assert("index=" + index + ", numBits=" + numBits);
        ...
        /// iterator.
        /// </summary>
        public void Or(DocIdSetIterator iter, int wordOffset = 0)
        {
            if (iter.DocID == -1 && iter is OpenBitSetIterator obs)
            {
                Or(obs.arr, obs.words, wordOffset);
            ...
            {
                Or(fbs.bits, fbs.numWords, wordOffset);
                // advance after last doc that would be accepted if standard
                // iteration is used (to exhaust it):
                fbs.Advance(numBits);
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Or(FixedBitSet<TBits> other, int wordOffset = 0)
        {
            Or(other.bits, other.numWords, wordOffset);
        }

        private void Or(Span<long> otherArr, int otherNumWords, int wordOffset)
        {
            ...
            int pos = Math.Min(numWords - wordOffset, otherNumWords);
            while (--pos >= 0)
            {
                thisArr[pos + wordOffset] |= otherArr[pos];
            }
        }

        /// <summary>
        /// this = this XOR other </summary>
        public void Xor(FixedBitSet<TBits> other)
        {
        ...
        public bool Intersects(FixedBitSet<TBits> other)
        {
            int pos = Math.Min(numWords, other.numWords);
            while (--pos >= 0)
            {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void And(FixedBitSet<TBits> other)
        {
            And(other.bits, other.numWords);
        }

        private void And(Span<long> otherArr, int otherNumWords)
        {
            Span<long> thisArr = this.bits;
            ...
            {
                thisArr.Slice(otherNumWords, this.numWords).Fill(0L);
            }
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void AndNot(FixedBitSet<TBits> other)
        {
            AndNot(other.bits, other.bits.Length);
        }

        private void AndNot(Span<long> otherArr, int otherNumWords)
        {
            Span<long> thisArr = this.bits;
            int pos = Math.Min(this.numWords, otherNumWords);
            while (--pos >= 0)
        ...
        public void Set(int startIndex, int endIndex)
        {
            ...
            bits[startWord] |= startmask;
            bits.Slice(startWord + 1, endWord).Fill(-1L);
            bits[endWord] |= endmask;
        }
        ...
        public void Clear(int startIndex, int endIndex) // LUCENENET TODO: API: Change this to use startIndex and length to match .NET
        {
            ...
            bits[startWord] &= startmask;
            bits.Slice(startWord + 1, endWord).Fill(0L);
            bits[endWord] &= endmask;
        }
        ...
        {
            return new FixedBitSet(bits.ToArray(), numBits);
        }
        ...
        public override bool Equals(object o)
        {
            ...
            if (!(o is FixedBitSet<TBits>))
            {
                return false;
            }
            var other = (FixedBitSet<TBits>)o;

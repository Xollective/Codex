...
public record struct NormalizedPath : IEquatable<NormalizedPath>, IStringConvertible<NormalizedPath>
{
    ...
    public string Extension => Path.SubstringAfterLastIndexOfAny("/").SubstringAfterLastIndexOfAny(".", requireMatch: true).ToString();
    ...
    public static implicit operator string(NormalizedPath value)
    {
        return value.Path;
    }
}
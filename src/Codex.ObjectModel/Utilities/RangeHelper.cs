...
namespace Codex.Utilities
{
    ...
    public static class RangeHelper
    {
        ...
        public static int BinarySearch<T>(this IReadOnlyList<T> list, T value, IComparer<T> comparer = null)
        {
            ...
            return InternalBinarySearch(list, 0, list.Count, value, comparer);
        }

        private static int InternalBinarySearch<T>(IReadOnlyList<T> array, int index, int length, T value, IComparer<T> comparer)
        {
            int lo = index;
        ...
        /// The matched range
        /// </returns>
        public static Extent GetRange<TItem, TRange, TList>(
            this TList list,
            TRange range,
            Func<TRange, TItem, int> minimumComparer,
            Func<TRange, TItem, int> maximumComparer,
            bool includeEquals = false)
            where TList : IReadOnlyList<TItem>
        {
            ...
            //int minCompare(TRange range, TItem item)
            //{
            //    var result = minCompare(range, item);
            ...
            return new Extent(lower, (upper - lower) + 1);
        }

...
namespace Lucene.Net.Search
{
    ...
    public class TermQuery : Query
    {
        ...
        internal sealed class TermWeight : Weight
        {
            ...
            {
                if (Debugging.AssertsEnabled) Debugging.Assert(termStates == null || termStates.TopReaderContext == ReaderUtil.GetTopLevelContext(context),"The top-reader used to create Weight ({0}) is not the same as the current reader's top-reader ({1})", termStates.TopReaderContext, ReaderUtil.GetTopLevelContext(context));
            ...
            private TermsEnum GetTermsEnum(AtomicReaderContext context)
            {
                ...
                if (termsEnum is null || !outerInstance.Seek(termsEnum, state))
                {
                    return null;
                }
                ...
                //{
                //    // This Debug.Assert can throw in normal cases because MultiTermQuery will not populate
                //    // the term states for all readers if it has already found enough term matches for its cutoff.
        ...
        public virtual bool PrebuildTermContext { get; set; } = true;

        public virtual bool Seek(TermsEnum termsEnum, TermState state)
        {
            if (state == null)
            {
                if (!PrebuildTermContext)
                {
                    return termsEnum.SeekExact(Term.Bytes);
                }
        ...
        public override Weight CreateWeight(IndexSearcher searcher)
        {
            ...
            if (!PrebuildTermContext)
            {
                termState = new TermContext(context);
            }
            else if (perReaderTermState is null || perReaderTermState.TopReaderContext != context)
            {

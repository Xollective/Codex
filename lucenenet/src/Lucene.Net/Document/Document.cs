...
namespace Lucene.Net.Documents
{
    ...
    public sealed class Document : IEnumerable<IIndexableField>, IFormattable
    {
        ...
        public static void NotifyWriteDocument(IEnumerable<IIndexableField> fields, SegmentInfo segmentInfo, int docId)
        {
            if (fields is Document doc)
            {
                doc.OnWriteDocument?.Invoke(segmentInfo, docId);
            }
        }

        public delegate void WriteDocumentHandler(SegmentInfo segmentInfo, int docId);

        /// <summary>
        /// Event triggered when document is written to a segment
        /// </summary>
        public WriteDocumentHandler OnWriteDocument;

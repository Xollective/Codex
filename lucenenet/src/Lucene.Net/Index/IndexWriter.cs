...
namespace Lucene.Net.Index
{
    ...
    public class IndexWriter : IDisposable, ITwoPhaseCommit
    {
        ...
        public virtual DirectoryReader GetReaderNoFlush()
        {
            EnsureOpen();
            ...
            try
            {
                lock (fullFlushLock)
                {
                    ...
                    try
                    {
                        ...
                        lock (this)
                        {
                            r = StandardDirectoryReader.Open(this, segmentInfos, false);
                        }
                    }
        ...
        {
            AddIndexes(readers, Array.Empty<SegmentCommitInfo>());
        }

        public virtual void AddIndexes(IndexReader[] readers, IEnumerable<SegmentCommitInfo> deletedSegments)
        {
            EnsureOpen();
            int numDocs = 0;
            ...
            try
            {
                ...
                try
                {
                    ...
                    foreach (var deletedSegment in deletedSegments)
                    {
                        segmentInfos.Remove(deletedSegment);
                    }
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            try
            {
                ...
                foreach (string file in info.GetFiles())
                {
                    ...
                    // [LanceC]TODO: This seems like the place to handle merging indexes
                    // by overriding Directory.Copy to just create a virtual pointer

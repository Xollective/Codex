...
#nullable restore
        ...
        public ReadOnlySpan<T> ItemSpan => heap.AsSpan(1, size);

        public ReadOnlyMemory<T> ItemMemory => heap.AsMemory(1, size);
        ...
        public Node? TopNode => GetNode(1);

        public struct Node
        {
            public Node(PriorityQueue<T> queue, int index)
            {
                Queue = queue;
                Index = index;
            }

            public T Value => Queue.heap[Index];
            public Node? LeftChild => Queue.GetNode(Index * 2);
            public Node? RightChild => Queue.GetNode((Index * 2) + 1);

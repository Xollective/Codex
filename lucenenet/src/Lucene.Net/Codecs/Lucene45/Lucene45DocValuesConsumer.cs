...
namespace Lucene.Net.Codecs.Lucene45
{
    ...
    public class Lucene45DocValuesConsumer : DocValuesConsumer // LUCENENET specific - removed IDisposable, it is already implemented in base class
    {
        ...
        /// <summary>
        /// Maximum number of ranges for <see cref="RANGE_COMPRESSED"/> format. </summary>
        public const int RANGE_COMPRESSED_MAX_COUNT = 64;
        ...
        /// <summary>
        /// Compressed serializing small set of ranges of values. </summary>
        public const int RANGE_COMPRESSED = 3;
...
#pragma warning restore CA2213 // Disposable fields should be disposed
        ...
        internal virtual void AddNumericField(FieldInfo field, IEnumerable<long?> values, bool optimizeStorage)
        {
            ...
            List<(long start, long end)> ranges = null;
            
            if (optimizeStorage)
            {
                uniqueValues = new JCG.HashSet<long>();
                ranges = field.DocValuesType == Index.DocValuesType.NUMERIC 
                    ? new List<(long start, long end)>() 
                    : null;
                (long start, long end) last = (0, long.MinValue);
                ...
                foreach (long? nv in values)
                {
                    ...
                    if (ranges != null)
                    {
                        if (v != last.end + 1 || last.start > last.end)
                        {
                            last = (v, v);
                            ranges.Add(last);
                        }
                        ...
                        if (ranges.Count >= RANGE_COMPRESSED_MAX_COUNT)
                        {
                            ranges = null;
                        }
                    }
            ...
            // The number of ranges should be log2(count)
            if (ranges != null)
            {
                format = RANGE_COMPRESSED;
            }
            else if (uniqueValues != null && (delta < 0L || PackedInt32s.BitsRequired(uniqueValues.Count - 1) < PackedInt32s.BitsRequired(delta)) && count <= int.MaxValue)
            {
            ...
            switch (format)
            {
                ...
                case DELTA_COMPRESSED:
                    ...
                    writer.Finish();
                    break;
                case RANGE_COMPRESSED:
                    meta.WriteVInt32(ranges.Count);
                    foreach (var range in ranges)
                    {
                        meta.WriteVInt64(range.start);
                        meta.WriteVInt32((int)(range.end - range.start));
                    }
                    break;

...
#pragma warning restore 612, 618
        ...
        internal static NumericEntry ReadNumericEntry(IndexInput meta)
        {
            ...
            switch (entry.format)
            {
                ...
                case Lucene45DocValuesConsumer.RANGE_COMPRESSED:
                    int rangeCount = meta.ReadVInt32();
                    if (rangeCount > Lucene45DocValuesConsumer.RANGE_COMPRESSED_MAX_COUNT)
                    {
                        throw new Exception($"RANGE_COMPRESSED cannot have more than {Lucene45DocValuesConsumer.RANGE_COMPRESSED_MAX_COUNT} ranges, input=" + meta);
                    }

                    entry.table = new long[rangeCount * 3];
                    int docStart = 0;
                    for (int i = 0; i < entry.table.Length; )
                    {
                        var start = meta.ReadVInt64();
                        var docCount = meta.ReadVInt32() + 1;

                        entry.table[i++] = docStart;
                        entry.table[i++] = start;
                        entry.table[i++] = docCount;
        ...
        internal virtual Int64Values GetNumeric(NumericEntry entry)
        {
            ...
            switch (entry.format)
            {
                ...
                case Lucene45DocValuesConsumer.RANGE_COMPRESSED:
                    return new RangeTableInt64Values(this, entry.table);
        ...
        private class RangeTableInt64Values : Int64Values, INumericDocValuesRange, IBinarySearchNumericDocValues
        {
            ...
            private static readonly IComparer<(long docStart, long start, long length)> ValueBinarySearchComparer
                = Comparer<(long docStart, long start, long length)>.Create((v1, v2) => v1.start.CompareTo(v2.start));

            public RangeTableInt64Values(Lucene45DocValuesProducer outerInstance, long[] table)
            {
                this.outerInstance = outerInstance;
                this.table = table;
                foreach (var v in GetSpan())
                {
                    MaxValue = Math.Max(MaxValue ?? long.MinValue, v.start + v.length - 1);
                    MinValue = Math.Min(MinValue ?? long.MaxValue, v.start);
                }
            }

            public override long Get(long docId)
            {
                var span = GetSpan();
                var index = span.BinarySearch((docId, long.MaxValue, long.MaxValue));
                if (index < 0)
                {
                    index = ~index - 1;
                }

                var range = span[index];
                var offset = docId - range.docStart;
                var value = range.start + offset;
            ...
            {
                var span = GetSpan();
                var index = span.BinarySearch((0L, value, 0L), ValueBinarySearchComparer);
                if (index < 0)
                {
                    index = ~index - 1;
                }
                ...
                var docId = range.docStart + offset;
                if (docId > maxDoc || docId < 0)
                {
                    return -1;
                }
            ...
            private ReadOnlySpan<(long docStart, long start, long length)> GetSpan()
            {
                return MemoryMarshal.Cast<long, (long, long, long)>(table.AsSpan());
            }
            ...
            public IEnumerable<(int DocId, long Value)> Enumerate()
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    {
                        yield return (DocId: (int)entry.docStart + j, Value: entry.start + j);
                    }
                }
            }
        }
